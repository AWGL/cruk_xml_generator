...
def main():
    ...
    if status == "s":
        info_dict = passed_data(database_parser, worksheet, sample, info_dict)
    elif status == "f":
    ...
    if authoriser not in allowed_authorisers:
        raise PermissionError (f"Authoriser {authoriser} is not a permitted authoriser for CRUK. Please try again or "
                        f"see the bioinformatics team to arrange to have your initials added to the list.")
