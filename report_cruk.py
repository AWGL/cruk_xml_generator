...
class ReportCRUK:
    ...
    def error_condition(self, error_text, error_type):
    ...
    def report_cruk(self):
        ...
        worksheet = "19-9999"
        authoriser = "mj"
        module_logger = self.ml.module_logger #TODO temp
        ...
        info_dict = self.data_always_required(database_parser, sample)

        # Gather correct information depending on option selected by user input
        if status == "s":
            info_dict = self.passed_data(worksheet, sample, info_dict)
        elif status == "f":
            info_dict = self.qc_fail_data(info_dict)
        elif status == "w":
            info_dict = self.removed_from_trial_data(info_dict)
        ...
        if authoriser not in allowed_authorisers:
            self.error_condition(f"Authoriser {authoriser} is not a permitted authoriser for CRUK. Please "
                                f"see the bioinformatics team.", PermissionError)
        ...
        module_logger.info(self.check_xml_data(sample_dict.get(sample)))
        ...
        write_report = GenerateReport(os.path.join(os.getcwd(), output_pdf), sample_dict.get(sample), self.ml, status)
